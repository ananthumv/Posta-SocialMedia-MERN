      ...
      <div className={classes.wrapper}>
        <div className={classes.top}>
          <Link to={`/profileDetail/${post?.user?._id}`} className={classes.topLeft}>
            <img src={`http://localhost:5000/images/${post?.user?.profileImg}`} className={classes.profileUserImg} />
            <div className={classes.profileMetadata}>
              <p className={classes.username}><span >{capitalizeFirstLetter(post.user.username)}</span></p>
              {post?.location && <div className={classes.location}> {post.location}</div>}
            </div>
          </Link>
          {
        ...
        <div className={`${classes.controls} ${showComment && classes.showComment}`}>
          <div className={classes.controlsLeft}>
            { 
             isLiked& isLiked ? <AiFillHeart onClick={handleLikePost} />: <AiOutlineHeart onClick={handleLikePost} />
            }
            {console.log(isLiked)}
